...
(() => {
    ...
    async function apiFetch(endpoint, options = {}) {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                if (res.status === 503 || res.status === 504 || (err.detail && err.detail.status === "degraded_performance")) {
                    handleOffline();
                    throw new Error("Network congestion detected. Switched to Offline Mode.");
                }
            ...
            if (error.name === 'AbortError' || error.message.includes('Failed to fetch')) {
                handleOffline();
                throw new Error("Network connection dropped. Operating in Offline Mode.");
            }
            throw error;
        }
    }
    ...
    function setLoading(btn, busy) {
        ...
        if (textSpan) textSpan.textContent = busy ? "Processing…" : (btn.dataset.label || "Submit");
    ...
    async function checkHealth() {
        try {
            ...
            DOM.healthBadge.setAttribute("aria-label", `System status: operational, version ${data.version}`);
    ...
    function addToIncidentLog(incident) {
        ...
        const time = new Date(incident.acknowledged_at || Date.now()).toLocaleTimeString();
    ...
    function renderIncidentResponse(data) {
        ...
        const units = (ta.response_units || []).map((u) => `<li>${escapeHtml(u)}</li>`).join("");
        const steps = (ta.action_steps || []).map((s) => `<li>${escapeHtml(s)}</li>`).join("");
        ...
        DOM.incidentResult.innerHTML = `
            <div class="result-card" role="region" aria-label="Incident triage result">
                ...
                <div class="result-section">
                    ...
                    <p style="font-size:var(--fs-sm);line-height:1.6;">${escapeHtml(ta.commander_notes)}</p>
                </div>` : ""}
    ...
    function renderIncidentError(message) {
        DOM.incidentResult.innerHTML = `
            <div class="result-card result-card--error" role="alert">
                <div class="result-card__title" style="color:var(--danger);">
                    <span aria-hidden="true">❌</span> Error
                </div>
                <p style="font-size:var(--fs-sm);">${escapeHtml(message)}</p>
            </div>
        `;
    }
    ...
    function renderFanResponse(data) {
        ...
        DOM.fanResult.innerHTML = `
            <div class="result-card" role="region" aria-label="Fan assistant response">
                ...
                <div class="result-section">
                    <p style="font-size:var(--fs-base);line-height:1.7;white-space:pre-wrap;">${escapeHtml(data.reply)}</p>
                </div>
    ...
    DOM.incidentForm.addEventListener("submit", async (e) => {
        e.preventDefault();
        if (!state.isOnline) {
            renderIncidentError("Network offline — cannot submit incident. Please try again when connection is restored.");
            return;
        }
    ...
    DOM.fanForm.addEventListener("submit", async (e) => {
        e.preventDefault();
        if (!state.isOnline) {
            renderFanError("Network offline — cannot reach assistant. Please visit the nearest Info Desk.");
            return;
        }
